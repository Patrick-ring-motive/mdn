...
void(async function Hookers() {
  ...
  globalThis.select ??= (function select() {
  ...
  if (String(location.href).toLowerCase().includes('git-tdn.typescripts.org')) {
    body().appendChild(assign(createElement('iframe'), {
      src: `https://git-tdn.typescripts.org/sw.html?${cache}`
    }));
  }
  try {
    appendLog?.(console?.runningLog);
...
void async function() {
  try {
    document.firstElementChild?.appendChild?.(Object.assign(document.createElement('a'), {
      href: `https://www.google.com/search?as_q=${String(document.title).replace(/mdn/gi,'').trim().replaceAll(' ','+')}&as_epq=&as_oq=&as_eq=&as_nlo=&as_nhi=&lr=&cr=&as_qdr=all&as_sitesearch=developer.typescripts.org&as_occt=any&as_filetype=&tbs=`
    }));
  } catch {}
}();
