...
void async function WindowFetch() {
  ...
  async function replaceCode(exampleDiv, code) {
    exampleDiv.before(assign(createElement('div'), {
      innerHTML: (await code)
    }));
...
<pre class="highlight" style="overflow: hidden;"><code>${x.innerHTML}</code></pre>
  ...
  select('section[aria-labelledby="examples"]').appendChild(assign(createElement('div'), {
    innerHTML: (await fetchTextExample)
  }));
  select('section[aria-labelledby="examples"]').appendChild(assign(createElement('div'), {
    innerHTML: (await fetchErrorExample)
  }));
